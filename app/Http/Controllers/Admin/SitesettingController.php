...
class SitesettingController extends BaseController {
   ...
   public function index()
   {
       ...
       if(!empty($data["search_key"])){
          ...
          $sitesettings = DB::table('sitesettings')->where('not_visible','=','0')->where('homepagevisiblestatus','=','0')->where('name','like',$search_key."%")->orderBy('id','desc')->paginate($limit);
        ...
        $search_key='';
        $sitesettings = DB::table('sitesettings')->where('not_visible','=','0')->where('homepagevisiblestatus','=','0')->orderBy('id','desc')->paginate($limit);
       }
       
		
        //echo '<pre>';print_r($vitamins); exit;
	    $sitesettings->setPath('sitesetting');
        return view('admin.sitesetting.index',compact('sitesettings','search_key'),array('title'=>'Site Settings Management','module_head'=>'Site Settings'));
