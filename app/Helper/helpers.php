...
class helpers  {
    ...
    public function get_last_query() {
        ...
        public function get_country_hotel_detls($country_id){
            $min_price = Hotel::where('country_id', '=', $country_id)->min('min_single_supplement_fee');
            return $min_price;
        }

        public function get_hotel_image($hotel_id = null){
            $hotel_image = HotelImage::where('hotel_id',$hotel_id)->where('primary_image','1')->first();
            if(isset($hotel_image)){
                return $hotel_image->hotel_image;
            }else{
                return false;
            }
            //return $hotel_image->hotel_image;
        ...
        public function get_hotel_prices($hotel_id = null){
            /*$hotel_price = HotelPrice::addSelect('package_price',\DB::raw("MIN(single_supplement_fee) AS single_supplement_fee"))
                                       ->where('hotel_id','=',$hotel_id);*/

            //$hotel_price = HotelPrice::where('hotel_id',$hotel_id)->min('single_supplement_fee');
            //$hotel_price = DB::table('hotel_prices')
                            //->addSelect('package_price',\DB::raw("MIN(single_supplement_fee) AS single_supplement_fee"))
                            //->where('hotel_id', $hotel_id)->min('single_supplement_fee');

            $sql = "SELECT * FROM `hotel_prices` WHERE `single_supplement_fee` = (SELECT MIN(`single_supplement_fee`) FROM ";
            $sql .= "`hotel_prices` WHERE `hotel_id`='".$hotel_id."')";
            $hotel_price = DB::select($sql);
